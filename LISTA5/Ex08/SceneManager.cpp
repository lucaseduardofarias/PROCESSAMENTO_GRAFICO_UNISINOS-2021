...
void SceneManager::initializeGraphics()
{
	...
	addShader("../shaders/Custom.vs", "../shaders/Custom.frag");
...
void SceneManager::render()
{
	...
	for (int i = 0; i < objects.size(); i++)
	{
		...
		if (i == 2)
		{
			...
			glUniform1i(glGetUniformLocation(shaders[6]->Program, "ourTexture2"), 1);
...
void SceneManager::setupScene()
{
	...
	shaders[2]->setVec3("corColorizadora", 0.0, 0.65, 0.0);
	...
	obj->setDimension(glm::vec3(200.0f, 200.0f, 1.0f));
	obj->setShader(shaders[4]); //Ex06
	objects.push_back(obj); //adiciona o segundo obj
	shaders[4]->setInt("limiar", 95);

	//Adicionando mais um
	obj = new GameObject;
	obj->setPosition(glm::vec3(500.0f, 100.0f, 0.0));
	obj->setDimension(glm::vec3(200.0f, 200.0f, 1.0f));
	obj->setShader(shaders[5]); //Ex06
	objects.push_back(obj); //adiciona o segundo obj
	shaders[5]->setInt("limiar", 125);
	...
	textureMaskID = loadTexture("../textures/aim.png");
...
void SceneManager::setupCamera2D() //TO DO: parametrizar aqui
{
	...
	float ratio = 1.0;
	float xMin = 0.0, xMax = 600.0, yMin = 0.0, yMax = 400.0, zNear = -1.0, zFar = 1.0;
...
unsigned int SceneManager::loadTexture(string filename)
{
	...
	if (data)
	{
		//int sizeImg = width * height * nrChannels;
		//for (int i = 0; i < sizeImg; i += nrChannels)
		//{
		//	data[i] = 0; // R
		//	data[i + 1] = 0; //G
		//	data[i + 2] = 255; //B
